...
public class Demo {
    ...
    static void run() {
        ...
        s.setDeviceConfigs(new DeviceConfig(0, 64, 6, 200, 10_000_000),
                new DeviceConfig(1, 24, 6, 1, 10_000_000));
        ...
        s.setOnTimeUpdateCallback((progress, solutions, duration) -> {
            System.out.format("\r\tprogress: %1.10f\tsolutions: %18d\tduration: %dms", progress,
                    solutions, duration);
        });
        s.setTerminationCallback((self) -> {
            System.out.println();
            System.out.println(
                    self.getSolutions() + " solutions found in " + self.getDuration() + "ms");
        });
        s.solve();
    }
}
