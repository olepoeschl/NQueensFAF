...
public class CLI implements Runnable {
    ...
    @Option(names = {"-d", "--show-devices"},
            description = "Show a list of all available OpenCL devices")
    private boolean showAvailableDevices;

    private File configFile;

    @Option(names = {"-c", "--config-file"}, paramLabel = "FILE", required = false,
            description = "Absolute path to the file containing the run configuration")
    ...
    @Option(names = {"-t", "--task-file"}, paramLabel = "FILE", required = false,
            description = "Absolute path to the file containing the task")
    ...
    @Option(names = {"-N", "--board-size"}, paramLabel = "INT", required = false,
            description = "Set the board size")
    private int N = -69;

    // for printing the progress in the progress callback
    private final String progressStringFormat =
            "\r%c\tprogress: %1.10f\tsolutions: %18d\tduration: %12s";
            ...
            System.out
                    .println(
                            AsciiTable.getTable(AsciiTable.BASIC_ASCII, devices,
                                    Arrays.asList(
                                            new Column().header("Index")
                                                    .headerAlign(HorizontalAlign.CENTER)
                                                    .dataAlign(HorizontalAlign.CENTER)
                                                    .with(device -> Integer
                                                            .toString(device.getIndex())),
                                            new Column().header("Vendor")
                                                    .headerAlign(HorizontalAlign.CENTER)
                                                    .dataAlign(HorizontalAlign.CENTER)
                                                    .with(device -> device.getVendor()),
                                            new Column().header("Device Name")
                                                    .headerAlign(HorizontalAlign.CENTER)
                                                    .dataAlign(HorizontalAlign.CENTER)
        ...
        try {
            ...
            solver.setOnTimeUpdateCallback((progress, solutions, duration) -> {
                ...
                System.out.format(progressStringFormat, loadingChars[loadingCharIdx++], progress,
                        solutions, getDurationPrettyString(duration));
            });
            solver.setTerminationCallback((self) -> {
                System.out.println();
                System.out.println("found " + self.getSolutions() + " solutions in "
                        + getDurationPrettyString(self.getDuration()) + " for board size "
                        + self.getN());
            });
            ...
            symSolver.setTerminationCallback((self) -> {
                System.out.println("(" + symSolver.getUniqueSolutionsTotal(solver.getSolutions())
                        + " unique solutions)");
            });
            symSolver.setN(N);
            symSolver.solve();
