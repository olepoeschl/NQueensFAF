...
public class SymSolver extends Solver {
    ...
    protected void run() {
        ...
        // the main diagonals can only be occupied, if the corresponding queen is in the middle of
        // the board
        // this can only be the case for odd N
        // the queen in the middle of the board never has to be set - if we reach the middle row, we
        // already found a solution
        rot180Solver(1, L, mid, L, 1, 0);
        rot90Solver(1, L, mid, L, 1, mid, 0, 0);

        // solver for 180 degree symmetric solutions counts every 180 symmetric solution 4 times and
        // every 90 degree symmetric solution 4 times
        // the solver for 90 degree symmetric solutions counts every 90 degree symmetric solution
        // double
        // following values are the unique solution number with their respective symmetry ONLY
    ...
    private void rot90Solver(int ld, int rd, int col, int ldbot, int rdbot, int row, int rowidx,
            int queens) {
        ...
        int free = ~(ld | rd | col | (ldbot >>> (N - 1 - 2 * rowidx))
                | (rdbot << (N - 1 - 2 * rowidx))) & mask;
        ...
        while ((free & mask) > 0) {
            ...
            rot90Solver((ld | bit | revbit) << 1, (rd | bit | revbit) >>> 1,
                    col | bit | revbit | rowbit | revrowbit, (ldbot | bit | revbit) >>> 1,
                    (rdbot | bit | revbit) << 1, row | rowbit | revrowbit | bit | revbit,
                    rowidx + 1, queens + 1);
        }
    }

    // similar to 90 degree symmetric solver, just with less extra constraints
    // main idea in both solvers is, to set a queen and also the queen that comes from rotation by
    // 180 degrees
    // realize occupation by solving board from top to bottom and vice versa simultaneously
    private void rot180Solver(int ld, int rd, int col, int ldbot, int rdbot, int rowidx) {
        ...
        while ((free & mask) > 0) {
            ...
            rot180Solver((ld | bit) << 1, (rd | bit) >>> 1, col | bit | revbit,
                    (ldbot | revbit) >>> 1, (rdbot | revbit) << 1, rowidx + 1);
        }
    }
