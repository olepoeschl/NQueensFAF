...
class CPUSolverThread extends Thread {
    ...
    // IMPORTANT: since the left and right col are occupied by the startConstalletaion, we only deal
    ...
    // the occupancy of each row is represented with integers in binary representation (1 occupied,
    // 0 free)
    // there are different recursive functions for different arrangements of the queens i,j,k,l on
    // the border
    // in order to reduce the amount of different cases we rotate and mirror the board in such a
    // way,
    // that the queen j in the last row is as close to the right corner as possible
    // this is done by the function jasmin (j as min)
    // we call this distance to the corner d and distinguish between d=0,d=1,d=2,d <small enough>
    // and d <big>
    // for d <small enough> the diagonal jl from queen j going upwards to the left can already be
    // set
    ...
    // in the last function of every case, respectively, we check check in both next rows, if there
    // are free spaces in the row

    // of course, when traveling over row k or l or both or jl or jr, we have to shift ld and rd by
    // 2 or 3 rows at once
    ...
    private void SQd1BklB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                if (nextfree > 0)
                    SQd1B(((ld | bit) << 3) | 1, ((rd | bit) >> 3) | L4, col | bit, row + 3,
                            nextfree);
            }
            return;
        }
    ...
    private void SQd1BkBlB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                nextfree = ~(((ld | bit) << 2) | ((rd | bit) >> 2) | (col | bit) | L3);
                if (nextfree > 0)
                    SQd1BlB(((ld | bit) << 2), ((rd | bit) >> 2) | L3, col | bit, row + 2,
                            nextfree);
            }
            return;
        }
    ...
    private void SQd1BlkB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                if (nextfree > 0)
                    SQd1B(((ld | bit) << 3) | 2, ((rd | bit) >> 3) | L3, col | bit, row + 3,
                            nextfree);
            }
            return;
        }
    ...
    private void SQd2BlkB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                if (nextfree > 0)
                    SQd2B(((ld | bit) << 3) | 2, ((rd | bit) >> 3) | L3, col | bit, row + 3,
                            nextfree);
            }
            return;
        }
    ...
    private void SQd2BklB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                if (nextfree > 0)
                    SQd2B(((ld | bit) << 3) | 1, ((rd | bit) >> 3) | L4, col | bit, row + 3,
                            nextfree);
            }
            return;
        }
    ...
    private void SQd2BkBlB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                if (nextfree > 0)
                    SQd2BlB(((ld | bit) << 2), ((rd | bit) >> 2) | (1 << (N3)), col | bit, row + 2,
                            nextfree);
            }
            return;
        }
    ...
    private void SQBkBlBjrB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                nextfree = ~(((ld | bit) << 2) | ((rd | bit) >> 2) | (col | bit) | (1 << (N3)));
                if (nextfree > 0)
                    SQBlBjrB(((ld | bit) << 2), ((rd | bit) >> 2) | (1 << (N3)), col | bit, row + 2,
                            nextfree);
            }
            return;
        }
    ...
    private void SQBklBjrB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                nextfree = ~(((ld | bit) << 3) | ((rd | bit) >> 3) | (col | bit) | L4 | 1);
                if (nextfree > 0)
                    SQBjrB(((ld | bit) << 3) | 1, ((rd | bit) >> 3) | L4, col | bit, row + 3,
                            nextfree);
            }
            return;
        }
    ...
    private void SQBlkBjrB(int ld, int rd, int col, int row, int free) {
        ...
        if (row == mark1) {
            while (free > 0) {
                ...
                nextfree = ~(((ld | bit) << 3) | ((rd | bit) >> 3) | (col | bit) | L3 | 2);
                if (nextfree > 0)
                    SQBjrB(((ld | bit) << 3) | 2, ((rd | bit) >> 3) | L3, col | bit, row + 3,
                            nextfree);
            }
            return;
        }
    ...
    public void run() {
        ...
        for (Constellation constellation : constellations) {
            ...
            if (j < N - 3) {
                jmark = j + 1;
                endmark = N - 2;
                // if the queen j is more than 2 columns away from the corner but the rd from the
                ...
                // if we have to set some queens first in order to reach the row N-1-jmark where the
                // rd from queen j
                // can be set
                else {
                    // k < l
