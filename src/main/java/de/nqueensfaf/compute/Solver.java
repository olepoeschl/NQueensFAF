 ...
 * Then the solver will automatically have the util functions like adding callbacks etc. and the
 * given basic solver class structure makes it easier.
 * </p>
 * 
 * @author olepo
 */
public abstract class Solver {
    ...
    /**
     * The {@link Solver} calls the initialization callbacks and starts the threads for continous
    ...
    /**
     * if true, the Solver automatically deletes a file created by the auto save function using only
     * used when {@link #autoSave} is true
     */
    private boolean autoDeleteEnabled = Config.getDefaultConfig().isAutoDeleteEnabled();
    /**
     * if true, the Solver automatically calls store() using {@link #autoSaveFilename} if a specific
     * progress percentage step is completed
     * 
    ...
    /**
     * the filename (format) used for the autosave feature. only used when {@link #autoSave} is true
     * 
     * @see #autoSavePercentageStep
     */
    ...
    /**
     * Saves the current progress of the {@link Solver} in a file so that it can be continued at
     * some time later.
     * 
     * @param filepath path/name of the file the progress should be written in (existent or non
     *        existent)
     * @throws IOException
     */
    ...
    /**
     * Reads the progress of an old run of the {@link Solver} and injects this state so that it can
     * be continued.
     * 
     ...
     */
    protected abstract void inject_(String filepath)
            throws IOException, ClassNotFoundException, ClassCastException;

    /**
     * States if the Solver has been injected and therefore contains injected values.
     * 
     * @return true if inject() was called and the Solver was not started or resetted since,
     *         otherwise false
     */
    public abstract boolean isInjected();

    /**
     * Resets the {@link Solver}; therefore, applies default values to all Solver state and progress
     * related variables.
     */
    ...
    public static <T extends Solver> T createSolverWithConfig(Config config) {
        ...
        switch (config.getType().toLowerCase()) {
            ...
            case "gpu":
                ...
                break;
            default:
                throw new IllegalArgumentException("Invalid config value '" + config.getType()
                        + "' for solver type: has to be 'cpu' or 'gpu'");
        }
    ...
    /**
     * Calls all initialization callbacks, then starts the {@link Solver}'s run()-method, then calls
     * all termination callbacks.
     * 
     * @throws InterruptedException if the time limit is exceeded while waiting for the Solverthread
     *         to shutdown after terminating (time limit is the bigger one of progressUpdateDelay
     *         and timeUpdateDelay)
    ...
    public final void solve() {
        ...
        try {
            ucExecutor.awaitTermination(
                    progressUpdateDelay > timeUpdateDelay ? progressUpdateDelay + 100
                            : timeUpdateDelay + 100,
    ...
    /**
     * Checks if all prepartions are done correctly. * @throws {@link IllegalStateException} if N
     * equals 0, the {@link Solver} is already running or if getProgress() >= 100
     */
    ...
    /**
     * Starts the threads that continously update the {@link Solver}'s duration and progress using
     * the related delay. The threads run until the {@link Solver} is finished.
     * 
     * @see #timeUpdateDelay
            ...
            ucExecutor.submit(() -> {
                ...
                while (isRunning()) {
                    if (getDuration() != tmpTime) {
                        onTimeUpdateCallback.onTimeUpdate(getProgress(), getSolutions(),
                                getDuration());
                        tmpTime = getDuration();
                    }
                    if (!isRunning())
            ...
            ucExecutor.submit(() -> {
                ...
                while (isRunning()) {
                    ...
                    if (!Float.isNaN(progress)
                            && (progress != tmpProgress || solutions != tmpSolutions)) {
                        onProgressUpdateCallback.onProgressUpdate(progress, solutions,
                                getDuration());
     ...
     * {@link #autoSaveFilename} the current Solver. Only used when {@link #autoSave} is true
     * 
     * @see #autoSaverThread
     */
    ...
    /**
     * Solves the N-Queens-problem in a very simple and easy way. Only for small N's, bigger N's
     * would take much longer with this method.
     * 
     ...
     */
    public final synchronized void store(String filepath)
     ...
     * @throws IllegalArgumentException
     */
    public final void inject(String filepath) throws IOException, ClassNotFoundException,
     ...
     * @throws IllegalArgumentException
     */
    public final void inject(File file) throws IOException, ClassNotFoundException,
            ClassCastException, IllegalArgumentException {
    ...
    /**
     * Adds a callback that will be executed on start of the {@link Solver}. The callbacks will be
     * called in reversed insertion order.
     * 
    ...
    /**
     * Adds a callback that will be executed on finish of the {@link Solver}. The callbacks will be
     * called in reversed insertion order.
     * 
     * @param r the callback as {@link Runnable}
     * @throws {@link IllegalArgumentException} if r is null
    ...
    public final void setN(int n) {
        ...
        if (n <= 0 || n > 31) {
            throw new IllegalArgumentException(
                    "Board size must be a number between 0 and 32 (not inclusive)");
        }
        N = n;
    }
    ...
    /**
     * Sets {@link #onProgressUpdateCallback} or deletes an existing callback if the parameter is
     * null.
     * 
     * @param onProgressUpdateCallback callback to be called on each progress update
     */
    public final void setOnProgressUpdateCallback(
     ...
     * 
     * @param autoSaveEnabled if true, enables automatic saving of the current Solver; if false,
     *        disables automatic saving (default value).
     */
     ...
     * 
     * @param autoDeleteEnabled if true, enables automatic deleting of the files created by the auto
     *        save function
     */
    ...
    public final void setAutoSavePercentageStep(int autoSavePercentageStep) {
        if (autoSavePercentageStep <= 0 || autoSavePercentageStep >= 100) {
            throw new IllegalArgumentException(
                    "progressUpdateDelay must be a number between 0 and 100");
        }
        this.autoSavePercentageStep = autoSavePercentageStep;
    }

    /**
     * Can be called before exiting the program. If the Solver is currently storing, this method
     * blocks until the storing is done.
     * 
