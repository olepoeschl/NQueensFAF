...
public class GPUSolver extends Solver {
    ...
    protected void run() {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            for (Device device : devices) {
                // build program
                String options =
                        "-D N=" + N + " -D WORKGROUP_SIZE=" + device.config.getWorkgroupSize();
                ...
                long xqueue = clCreateCommandQueue(device.context, device.id,
                        CL_QUEUE_PROFILING_ENABLE, errBuf);
                checkCLError(errBuf);
                device.xqueue = xqueue;
                long memqueue = clCreateCommandQueue(device.context, device.id,
                        CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, errBuf);
                checkCLError(errBuf);
    ...
    private ArrayList<Constellation> fillWithTrash(List<Constellation> subConstellations,
            int workgroupSize) {
    ...
    private void addTrashConstellation(ArrayList<Constellation> constellations, int ijkl) {
        constellations.add(new Constellation(-1, (1 << N) - 1, (1 << N) - 1, (1 << N) - 1,
                (69 << 20) | ijkl, -2));
    }
    ...
    private void createContextsAndPrograms(MemoryStack stack, IntBuffer errBuf) {
        ...
        for (long platform : platforms) {
            List<Device> platformDevices = devices.stream()
                    .filter(device -> device.platform == platform).collect(Collectors.toList());
            ...
            long program = clCreateProgramWithSource(context, getKernelSourceAsString("kernels.c"),
                    errBuf);
            checkCLError(errBuf);
    ...
    private void runDevice(MemoryStack stack, IntBuffer errBuf, Device device) {
        ...
        int deviceCurrentWorkloadSize = device.config.getMaxGlobalWorkSize()
                / device.config.getWorkgroupSize() * device.config.getWorkgroupSize();
        ...
        while (ptr < device.constellations.size()) {
            if (ptr == 0) { // first workload -> create buffers
                device.workloadConstellations =
                        device.constellations.subList(ptr, ptr + deviceCurrentWorkloadSize);
    ...
    private void createBuffers(IntBuffer errBuf, Device device) {
        ...
        device.startijklMem =
                clCreateBuffer(device.context, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR,
                        device.workloadGlobalWorkSize * 4, errBuf);
        checkCLError(errBuf);
    ...
    private void fillBuffers(IntBuffer errBuf, Device device) {
        ...
        // ld
        ByteBuffer ldPtr = clEnqueueMapBuffer(device.memqueue, device.ldMem, true, CL_MAP_WRITE, 0,
        ...
        // rd
        ByteBuffer rdPtr = clEnqueueMapBuffer(device.memqueue, device.rdMem, true, CL_MAP_WRITE, 0,
                globalWorkSize * 4, null, null, errBuf, null);
        checkCLError(errBuf);
        for (int i = 0; i < globalWorkSize; i++) {
        ...
        // col
        ByteBuffer colPtr = clEnqueueMapBuffer(device.memqueue, device.colMem, true, CL_MAP_WRITE,
                0, globalWorkSize * 4, null, null, errBuf, null);
        checkCLError(errBuf);
        for (int i = 0; i < globalWorkSize; i++) {
        ...
        // startijkl
        ByteBuffer startijklPtr = clEnqueueMapBuffer(device.memqueue, device.startijklMem, true,
                CL_MAP_WRITE, 0, globalWorkSize * 4, null, null, errBuf, null);
        checkCLError(errBuf);
        for (int i = 0; i < globalWorkSize; i++) {
            startijklPtr.putInt(i * 4, workloadConstellations.get(i).getStartijkl());
        }
        checkCLError(clEnqueueUnmapMemObject(device.memqueue, device.startijklMem, startijklPtr,
                null, null));

        // result memory
        device.resPtr = clEnqueueMapBuffer(device.memqueue, device.resMem, true, CL_MAP_WRITE, 0,
                globalWorkSize * 8, null, null, errBuf, null);
        checkCLError(errBuf);
        for (int i = 0; i < globalWorkSize; i++) {
            device.resPtr.putLong(i * 8, workloadConstellations.get(i).getSolutions());
        }
        checkCLError(
                clEnqueueUnmapMemObject(device.memqueue, device.resMem, device.resPtr, null, null));
    ...
    private void explosionBoost9000(IntBuffer errBuf, Device device) {
        ...
        final PointerBuffer xEventBuf = BufferUtils.createPointerBuffer(1); // buffer for event that
                                                                            ...
                                                                            // measuring the
                                                                            // execution time
        checkCLError(clEnqueueNDRangeKernel(device.xqueue, device.kernel, dimensions, null,
                globalWorkSize, localWorkSize, null, xEventBuf));
        ...
        if (device.vendor.toLowerCase().contains("advanced micro devices")
            ...
            checkCLError(clEnqueueNDRangeKernel(device.xqueue, nullKernel, dimensions, null,
                    globalWorkSize, localWorkSize, null, null));
        }

        // get exact time values using CLEvent
        device.xEvent = xEventBuf.get(0);
        checkCLError(clSetEventCallback(device.xEvent, CL_COMPLETE, device.profilingCB =
                CLEventCallback.create((event, event_command_exec_status, user_data) -> {
                    LongBuffer startBuf = BufferUtils.createLongBuffer(1),
                            endBuf = BufferUtils.createLongBuffer(1);
                    int err = clGetEventProfilingInfo(event, CL_PROFILING_COMMAND_START, startBuf,
                            null);
                    checkCLError(err);
    ...
    private void readResults(Device device) {
        ...
        checkCLError(clEnqueueReadBuffer(device.memqueue, device.resMem, true, 0, device.resPtr,
                null, null));
        for (int i = 0; i < device.workloadGlobalWorkSize; i++) {
            if (device.workloadConstellations.get(i).getStartijkl() >> 20 == 69) // start=69 is for
                                                                                 ...
                                                                                 // constellations
                continue;
            long solutionsForConstellation = device.resPtr.getLong(i * 8) * symmetry(
                    device.workloadConstellations.get(i).getStartijkl() & 0b11111111111111111111);
    ...
    protected void store_(String filepath) throws IOException {
        ...
        out.writeValue(new File(filepath), new SolverState(N,
                System.currentTimeMillis() - start + storedDuration, constellations));
    }

    @Override
    protected void inject_(String filepath)
    ...
    public float getProgress() {
        ...
        return constellations.size() > 0 ? (float) solvedConstellations / constellations.size()
                : 0f;
    }
    ...
    private void fetchAvailableDevices(MemoryStack stack) {
        ...
        for (int p = 0; p < platforms.capacity(); p++) {
            ...
            checkCLError(
                    clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, devicesBuf, (IntBuffer) null));
            for (int d = 0; d < devicesBuf.capacity(); d++) {
                long deviceId = devicesBuf.get(d);
                Device device = new Device(deviceId, platform,
                        getDeviceInfoStringUTF8(deviceId, CL_DEVICE_VENDOR),
    ...
    public List<DeviceInfo> getAvailableDevices() {
        ...
        for (int i = 0; i < availableDevices.size(); i++) {
            deviceInfos.add(new DeviceInfo(i, availableDevices.get(i).vendor,
                    availableDevices.get(i).name));
        }
        return deviceInfos;
    }
    ...
    public void setDeviceConfigs(DeviceConfig... deviceConfigsInput) {
        ...
        for (DeviceConfig deviceConfig : deviceConfigsInput) {
            if (deviceConfig.getWeight() == 0)
                continue;
            if (deviceConfigsTmp.stream()
                    .anyMatch(dvcCfg -> deviceConfig.getIndex() == dvcCfg.getIndex())) // check for
                                                                                       // duplicates
                continue;
    ...
    private String getKernelSourceAsString(String filepath) {
        ...
        try (InputStream clSourceFile =
                GPUSolver.class.getClassLoader().getResourceAsStream(filepath);
    ...
    private boolean symmetry90(int ijkl) {
        if (((geti(ijkl) << 15) + (getj(ijkl) << 10) + (getk(ijkl) << 5)
                + getl(ijkl)) == (((N - 1 - getk(ijkl)) << 15) + ((N - 1 - getl(ijkl)) << 10)
                        + (getj(ijkl) << 5) + geti(ijkl)))
            return true;
        return false;
    }
