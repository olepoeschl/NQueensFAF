...
class GPUConstellationsGenerator {
    ...
    public void genConstellations(int N, int preQueens) {
        ...
        for (int k = 1; k < N - 2; k++) { // j is idx of Queen in last row
            for (int i = k + 1; i < N - 1; i++) { // l is idx of Queen in last col
                ...
                // queens i and l
                rd = (L >>> (i + 1)) | (L >>> 1);
                // left border from k, right border from l, also bits i and j from the corresponding
                // queens
                col = 1 | L | (L >>> i);
    ...
    // TODO
    public List<Constellation> setMorePreQueens(List<Constellation> constellations,
            int moreQueens) {
        return null;
    }
