...
kernel void nqfaf_nvidia(global struct constellation *constellation_arr, global uint* jkl_queens_arr, global long *result) {
	...
	local uint stack[4*WORKGROUP_SIZE*N];
	stack[4*l_id*N + 4*start] = ld;
	stack[4*l_id*N + 4*start+1] = rd;
	stack[4*l_id*N + 4*start+2] = col;
	stack[4*l_id*N + 4*start+3] = free;
	...
	while(row >= start) {				// while we haven't tried everything
		...
		if(free){
			queen = free & -free;
			stack[4*l_id*N + 4*row+3] = free ^ queen;
			ld = (ld | queen) << 1;
			rd = (rd | queen) >> 1;
			col |= queen;
			row++;
			stack[4*l_id*N + 4*row] = ld;
			stack[4*l_id*N + 4*row+1] = rd;
			stack[4*l_id*N + 4*row+2] = col;
			stack[4*l_id*N + 4*row+3] = free;
			free = ~(ld | rd | col | jkl_queens[row]);
		}
		else{
			row--;
			ld = stack[4*l_id*N + 4*row];
			rd = stack[4*l_id*N + 4*row+1];
			col = stack[4*l_id*N + 4*row+2];
			free = stack[4*l_id*N + 4*row+3];
		}
	}
