...
//	int j = (c.start_ijkl >> 10) & 31;	// queen in last row at position j
//	int k = (c.start_ijkl >> 5) & 31;	// in row k queen at left border, in row l queen at right border
//	int l = c.start_ijkl & 31;
	...
	queens[get_local_id(0)][start] = queen;			// we already calculated the first queen in the start row 
	...
	while(row >= start) {				// while we haven't tried everything 
		if(free) {					// if there are free slots in the current row 
			...
			queens[get_local_id(0)][row] = queen;			// remember the queen 
			...
			queen = queens[get_local_id(0)][row];			// recover the queen in order to remove it 
