...
public class SymSolver extends Solver{
	...
	protected void run() {
		...
		// the main diagonals can only be occupied, if the corresponding queen is in the middle of the board
		// this can only be the case for odd N
		// the queen in the middle of the board never has to be set - if we reach the middle row, we already found a solution 
		rot180Solver(1, L, mid, L, 1, 0);
		rot90Solver(1, L, mid, L, 1, mid, 0, 0);
		
		// solver for 180 degree symmetric solutions counts every 180 symmetric solution 4 times and every 90 degree symmetric solution 4 times 
		// the solver for 90 degree symmetric solutions counts every 90 degree symmetric solution double 
	...
	// main idea in both solvers is, to set a queen and also the queen that comes from rotation by 180 degrees
